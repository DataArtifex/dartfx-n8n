        ...
        options: [
          {
            name: 'Apply (apply)',
            value: 'apply',
            description: 'Apply a series of transformation functions to given CSV column/s. This can be used to perform typical data-wrangling tas',
            action: 'Apply',
          },
          {
            name: 'Behead (behead)',
            value: 'behead',
            description: 'Drop a CSV file\'s header. See also https://github.com/dathere/qsv/wiki/Transform-and-Reshape#behead',
            action: 'Behead',
          },
          {
            name: 'Blake3 (blake3)',
            value: 'blake3',
            description: 'Compute cryptographic hashes of files using blake3. This command is functionally similar to b3sum, providing fast, paral',
            action: 'Blake3',
          },
          {
            name: 'Cat (cat)',
            value: 'cat',
            description: 'Concatenate CSV files by row or by column. When concatenating by column, the columns will be written in the same order a',
            action: 'Cat',
          },
          {
            name: 'Clean (clean)',
            value: 'clean',
            description: 'Removes qsv-generated cache files to reduce clutter and simplify data packaging. By default, clean removes the three aut',
            action: 'Clean',
          },
          {
            name: 'Clipboard (clipboard)',
            value: 'clipboard',
            description: 'Provide input from the clipboard or save output to the clipboard. Note when saving to clipboard on Windows, line breaks ',
            action: 'Clipboard',
          },
          {
            name: 'Color (color)',
            value: 'color',
            description: 'Outputs tabular data as a pretty, colorized table that always fits into the terminal. Tabular data formats include CSV a',
            action: 'Color',
          },
          {
            name: 'Count (count)',
            value: 'count',
            description: 'Returns a count of the number of records in the CSV data. It has three modes of operation: Note that the count will not ',
            action: 'Count',
          },
          {
            name: 'Datefmt (datefmt)',
            value: 'datefmt',
            description: 'Formats recognized date fields (19 formats recognized) to a specified date format using strftime date format specifiers.',
            action: 'Datefmt',
          },
          {
            name: 'Dedup (dedup)',
            value: 'dedup',
            description: 'Deduplicates CSV rows. This requires reading all of the CSV data into memory because because the rows need to be sorted ',
            action: 'Dedup',
          },
          {
            name: 'Denull (denull)',
            value: 'denull',
            description: 'Detect null sentinels - literal text like "NULL" or "N/A" standing in for a missing value - that stop a numeric column f',
            action: 'Denull',
          },
          {
            name: 'Describegpt (describegpt)',
            value: 'describegpt',
            description: 'Create a "neuro-symbolic" Data Dictionary and/or infer Description & Tags about a Dataset using an OpenAI API-compatible',
            action: 'Describegpt',
          },
          {
            name: 'Diff (diff)',
            value: 'diff',
            description: 'Find the difference between two CSVs with ludicrous speed. NOTE: diff does not support stdin. A file path is required fo',
            action: 'Diff',
          },
          {
            name: 'Edit (edit)',
            value: 'edit',
            description: 'Replace the value of a cell specified by its row and column. Example: items.csv ```csv item,color shoes,blue flashlight,',
            action: 'Edit',
          },
          {
            name: 'Enum (enum)',
            value: 'enum',
            description: 'Add a new column enumerating the lines of a CSV file. This can be useful to keep track of a specific line order, give a ',
            action: 'Enum',
          },
          {
            name: 'Excel (excel)',
            value: 'excel',
            description: 'Exports a specified Excel/ODS sheet to a CSV file. The first non-empty row of a sheet is assumed to be the header row. E',
            action: 'Excel',
          },
          {
            name: 'Exclude (exclude)',
            value: 'exclude',
            description: 'Removes a set of CSV data from another set based on the specified columns. Also can compute the intersection of two CSV ',
            action: 'Exclude',
          },
          {
            name: 'Explode (explode)',
            value: 'explode',
            description: 'Explodes a row into multiple ones by splitting a column value based on the given separator. Example: ```csv name,colors ',
            action: 'Explode',
          },
          {
            name: 'Extdedup (extdedup)',
            value: 'extdedup',
            description: 'Remove duplicate rows from an arbitrarily large CSV/text file using a memory-mapped, on-disk hash table. Unlike the \'de',
            action: 'Extdedup',
          },
          {
            name: 'Extsort (extsort)',
            value: 'extsort',
            description: 'Sort an arbitrarily large CSV/text file using a multithreaded external sort algorithm. This command has TWO modes of ope',
            action: 'Extsort',
          },
          {
            name: 'Fetch (fetch)',
            value: 'fetch',
            description: 'Send/Fetch data to/from web services for every row using HTTP Get. Fetch is integrated with `jaq` (a jq clone) to direct',
            action: 'Fetch',
          },
          {
            name: 'Fetchpost (fetchpost)',
            value: 'fetchpost',
            description: 'Fetchpost sends/fetches data to/from web services for every row using HTTP Post. As opposed to fetch, which uses HTTP Ge',
            action: 'Fetchpost',
          },
          {
            name: 'Fill (fill)',
            value: 'fill',
            description: 'Fill empty fields in selected columns of a CSV. This command fills empty fields in the selected column using the last se',
            action: 'Fill',
          },
          {
            name: 'Fixedwidth (fixedwidth)',
            value: 'fixedwidth',
            description: 'Converts fixed-width text (fields at fixed byte-column positions, no delimiters) to CSV. By default, this expects the in',
            action: 'Fixedwidth',
          },
          {
            name: 'Fixlengths (fixlengths)',
            value: 'fixlengths',
            description: 'Transforms CSV data so that all records have the same length. The length is the length of the longest record in the data',
            action: 'Fixlengths',
          },
          {
            name: 'Flatten (flatten)',
            value: 'flatten',
            description: 'Prints flattened records such that fields are labeled separated by a new line. This mode is particularly useful for view',
            action: 'Flatten',
          },
          {
            name: 'Fmt (fmt)',
            value: 'fmt',
            description: 'Formats CSV data with a custom delimiter or CRLF line endings. Generally, all commands in qsv output CSV data in a defau',
            action: 'Fmt',
          },
          {
            name: 'Foreach (foreach)',
            value: 'foreach',
            description: 'Execute a shell command once per record in a given CSV file. NOTE: Windows users are recommended to use Git Bash as thei',
            action: 'Foreach',
          },
          {
            name: 'Frequency (frequency)',
            value: 'frequency',
            description: 'Compute a frequency distribution table on input data. It has CSV and JSON output modes. https://en.wikipedia.org/wiki/Fr',
            action: 'Frequency',
          },
          {
            name: 'Get (get)',
            value: 'get',
            description: 'Get tabular data from various sources into a managed, queryable disk cache. `get` fetches a resource once, stores it com',
            action: 'Get',
          },
          {
            name: 'Geocode (geocode)',
            value: 'geocode',
            description: 'Geocodes a location in CSV data against an updatable local copy of the Geonames cities index and a local copy of the Max',
            action: 'Geocode',
          },
          {
            name: 'Geoconvert (geoconvert)',
            value: 'geoconvert',
            description: 'Convert between various spatial formats and CSV/SVG including GeoJSON, SHP, and more. For example to convert a GeoJSON f',
            action: 'Geoconvert',
          },
          {
            name: 'Headers (headers)',
            value: 'headers',
            description: 'Prints the fields of the first row in the CSV data. These names can be used in commands like \'select\' to refer to colu',
            action: 'Headers',
          },
          {
            name: 'Implode (implode)',
            value: 'implode',
            description: 'Implodes multiple rows into one by grouping on key column(s) and joining the values of another column with the given sep',
            action: 'Implode',
          },
          {
            name: 'Index (index)',
            value: 'index',
            description: 'Creates an index of the given CSV data, which can make other operations like slicing, splitting and gathering statistics',
            action: 'Index',
          },
          {
            name: 'Input (input)',
            value: 'input',
            description: 'Read CSV data with special commenting, quoting, trimming, line-skipping & non UTF-8 encoding rules and transforms it to ',
            action: 'Input',
          },
          {
            name: 'Join (join)',
            value: 'join',
            description: 'Joins two sets of CSV data on the specified columns. The default join operation is an \'inner\' join. This corresponds t',
            action: 'Join',
          },
          {
            name: 'Joinp (joinp)',
            value: 'joinp',
            description: 'Joins two sets of CSV data on the specified columns using the Polars engine. The default join operation is an \'inner\' ',
            action: 'Joinp',
          },
          {
            name: 'Json (json)',
            value: 'json',
            description: 'Convert JSON to CSV. The JSON data is expected to be non-empty and non-nested as either: 1. An array of objects where: 2',
            action: 'Json',
          },
          {
            name: 'Jsonl (jsonl)',
            value: 'jsonl',
            description: 'Convert newline-delimited JSON (JSONL/NDJSON) to CSV. The command tries to do its best but since it is not possible to s',
            action: 'Jsonl',
          },
          {
            name: 'Lens (lens)',
            value: 'lens',
            description: 'Explore tabular data files interactively using the csvlens (https://github.com/YS-L/csvlens) engine. If the polars featu',
            action: 'Lens',
          },
          {
            name: 'Luau (luau)',
            value: 'luau',
            description: 'Create multiple new computed columns, filter rows or compute aggregations by executing a Luau 0.728 script for every row',
            action: 'Luau',
          },
          {
            name: 'Log (log)',
            value: 'log',
            description: 'Logs an MCP tool invocation entry to qsvmcp.log. Only intended for internal use by the qsv MCP server, not for general C',
            action: 'Log',
          },
          {
            name: 'Moarstats (moarstats)',
            value: 'moarstats',
            description: 'Add dozens of additional statistics, including extended outlier, robust & bivariate statistics to an existing stats CSV ',
            action: 'Moarstats',
          },
          {
            name: 'Partition (partition)',
            value: 'partition',
            description: 'Partitions the given CSV data into chunks based on the value of a column. See `split` command to split a CSV data by row',
            action: 'Partition',
          },
          {
            name: 'Pivotp (pivotp)',
            value: 'pivotp',
            description: 'Pivots or groups CSV data using the Polars engine. PIVOT MODE (with <on-cols>): GROUP-BY MODE (without <on-cols>): For e',
            action: 'Pivotp',
          },
          {
            name: 'Pragmastat (pragmastat)',
            value: 'pragmastat',
            description: 'Pragmatic statistical toolkit. Compute robust, median-of-pairwise statistics from the Pragmastat library. Designed for m',
            action: 'Pragmastat',
          },
          {
            name: 'Pro (pro)',
            value: 'pro',
            description: 'Interact with qsv pro API. Learn more about qsv pro at: https://qsvpro.dathere.com. - qsv pro must be running for this c',
            action: 'Pro',
          },
          {
            name: 'Profile (profile)',
            value: 'profile',
            description: 'Profile a CSV (local path or URL) and emit a `.metadata.json` file carrying five top-level blocks: Behind the scenes qsv',
            action: 'Profile',
          },
          {
            name: 'Prompt (prompt)',
            value: 'prompt',
            description: 'Open a file dialog to pick a file as input or save to an output file. Examples: See also https://github.com/dathere/qsv/',
            action: 'Prompt',
          },
          {
            name: 'Pseudo (pseudo)',
            value: 'pseudo',
            description: 'Pseudonymise the value of a given column by replacing it with an incremental identifier. See https://en.wikipedia.org/wi',
            action: 'Pseudo',
          },
          {
            name: 'Rename (rename)',
            value: 'rename',
            description: 'Rename the columns of a CSV efficiently. It has two modes of operation: Positional mode (default): The new column names ',
            action: 'Rename',
          },
          {
            name: 'Replace (replace)',
            value: 'replace',
            description: 'Replace occurrences of a pattern across a CSV file. You can of course match groups using parentheses and use those in th',
            action: 'Replace',
          },
          {
            name: 'Reverse (reverse)',
            value: 'reverse',
            description: 'Reverses rows of CSV data. Useful for cases when there is no column that can be used for sorting in reverse order, or wh',
            action: 'Reverse',
          },
          {
            name: 'Safenames (safenames)',
            value: 'safenames',
            description: 'Modify headers of a CSV to only have "safe" names - guaranteed "database-ready" names (optimized specifically for Postgr',
            action: 'Safenames',
          },
          {
            name: 'Sample (sample)',
            value: 'sample',
            description: 'Randomly samples CSV data. It supports ten sampling methods: * RESERVOIR: the default sampling method when NO INDEX is p',
            action: 'Sample',
          },
          {
            name: 'Schema (schema)',
            value: 'schema',
            description: 'Generate JSON Schema or Polars Schema (with the `--polars` option) from CSV data. JSON Schema Validation: ==============',
            action: 'Schema',
          },
          {
            name: 'Search (search)',
            value: 'search',
            description: 'Filters CSV data by whether the given regex matches a row. The regex is applied to selected field in each row, and if an',
            action: 'Search',
          },
          {
            name: 'Searchset (searchset)',
            value: 'searchset',
            description: 'Filters CSV data by whether the given regex set matches a row. Unlike the search operation, this allows regex matching o',
            action: 'Searchset',
          },
          {
            name: 'Select (select)',
            value: 'select',
            description: 'Select columns from CSV data efficiently. This command lets you manipulate the columns in CSV data. You can re-order, du',
            action: 'Select',
          },
          {
            name: 'Slice (slice)',
            value: 'slice',
            description: 'Returns the rows in the range specified (starting at 0, half-open interval). The range does not include headers. If the ',
            action: 'Slice',
          },
          {
            name: 'Snappy (snappy)',
            value: 'snappy',
            description: 'Does streaming compression/decompression of the input using the Snappy framing format. https://github.com/google/snappy/',
            action: 'Snappy',
          },
          {
            name: 'Sniff (sniff)',
            value: 'sniff',
            description: 'Quickly sniff the first n rows and infer CSV metadata (delimiter, header row, number of preamble rows, quote character, ',
            action: 'Sniff',
          },
          {
            name: 'Sort (sort)',
            value: 'sort',
            description: 'Sorts CSV data in lexicographical, natural, numerical, reverse, unique or random order. Note that this requires reading ',
            action: 'Sort',
          },
          {
            name: 'Sortcheck (sortcheck)',
            value: 'sortcheck',
            description: 'Check if a CSV is sorted. The check is done on a streaming basis (i.e. constant memory). With the --json options, also r',
            action: 'Sortcheck',
          },
          {
            name: 'Split (split)',
            value: 'split',
            description: 'Splits the given CSV data into chunks. It has three modes: by size (rowcount), by number of chunks and by kb-size. See `',
            action: 'Split',
          },
          {
            name: 'Scoresql (scoresql)',
            value: 'scoresql',
            description: 'Analyze a SQL query against CSV file caches (stats, moarstats, frequency) to produce a performance score with actionable',
            action: 'Scoresql',
          },
          {
            name: 'Sqlp (sqlp)',
            value: 'sqlp',
            description: 'Run blazing-fast Polars SQL queries against several CSVs - replete with joins, aggregations, grouping, table functions, ',
            action: 'Sqlp',
          },
          {
            name: 'Stats (stats)',
            value: 'stats',
            description: 'Compute summary statistics & infers data types for each column in a CSV. IMPORTANT: `stats` is heavily optimized for spe',
            action: 'Stats',
          },
          {
            name: 'Synthesize (synthesize)',
            value: 'synthesize',
            description: 'Generates a synthetic CSV that is statistically faithful to a source CSV. `synthesize` analyzes <input> with `stats` and',
            action: 'Synthesize',
          },
          {
            name: 'Table (table)',
            value: 'table',
            description: 'Outputs CSV data as a table with columns in alignment. Though this command is primarily designed for DISPLAYING CSV data',
            action: 'Table',
          },
          {
            name: 'Template (template)',
            value: 'template',
            description: 'Renders a template using CSV data with the MiniJinja template engine. https://docs.rs/minijinja/latest/minijinja/ This c',
            action: 'Template',
          },
          {
            name: 'Tojsonl (tojsonl)',
            value: 'tojsonl',
            description: 'Smartly converts CSV to a newline-delimited JSON (JSONL/NDJSON). By computing stats on the CSV first, it "smartly" infer',
            action: 'Tojsonl',
          },
          {
            name: 'To (to)',
            value: 'to',
            description: 'Convert CSV files to Parquet, PostgreSQL, SQLite, Excel XLSX, ODS and Data Package. PARQUET ======= Convert CSV files to',
            action: 'To',
          },
          {
          ...
          },
          {
            ...
            value: 'validate',
            description: 'Validates CSV data using two main modes: JSON SCHEMA VALIDATION MODE: =========================== This mode is invoked i',
            action: 'Validate',
          },
          {
            name: 'Viz (viz)',
            value: 'viz',
            description: 'Generate charts/maps from CSV data using the plotly charting library. Produces a self-contained, interactive HTML chart ',
            action: 'Viz',
          },
        ],
        default: 'stats',
