...
export async function executePartition(
  ...
  if (!column || !String(column).trim()) {
    throw new NodeOperationError(
      this.getNode(),
      'Parameter "Column" is required for partition.',
      { itemIndex },
    );
  }
  if (!outdir || !String(outdir).trim()) {
    throw new NodeOperationError(
      this.getNode(),
      'Parameter "Output Directory" is required for partition.',
      { itemIndex },
    );
  }
  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `Please ensure 'qsv' is installed and available in the system PATH where n8n is running, or specify its absolute path via the DARTFX_QSV_BIN_PATH environment variable. (Docs: https://github.com/dathere/qsv/blob/master/docs/help/partition.md)`,
        },
      );
    }
    ...
    if (error.code === 'ERR_CHILD_PROCESS_STDIO_MAXBUFFER' || (error.message && error.message.includes('maxBuffer'))) {
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `qsv partition returned more data than could fit into memory. Specify an 'Output File Path' to stream results directly to disk instead.`,
        },
      );
    }
    ...
    if (
      ...
      throw new NodeOperationError(
        this.getNode(),
        `Operation 'partition' is not available in the installed QSV binary`,
        {
          itemIndex,
          description: `The installed QSV binary at '${qsvBin}' does not include the 'partition' feature. This feature requires a QSV build with the corresponding Cargo feature enabled (or 'all_features'). See https://github.com/dathere/qsv/blob/master/docs/help/partition.md and https://github.com/dathere/qsv#feature-flags`,
        },
      );
    }
    ...
    if (rawError.includes('No such file or directory') || rawError.includes('os error 2')) {
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `qsv partition could not find the file at '${inputPath}'. Check for typos, or if n8n is running in Docker, ensure the host directory is mounted into the container.`,
        },
      );
    }
    ...
    if (
      ...
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `qsv partition was denied read access to '${inputPath}'. On macOS, check Full Disk Access or Removable Volumes permissions for the application running n8n.`,
        },
      );
    }
    ...
    throw new NodeOperationError(
      this.getNode(),
      `Failed executing 'qsv partition': ${rawError}`,
      { itemIndex },
    );
  }
}
