  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `Please ensure 'qsv' is installed and available in the system PATH where n8n is running, or specify its absolute path via the DARTFX_QSV_BIN_PATH environment variable. (Docs: https://github.com/dathere/qsv/blob/master/docs/help/flatten.md)`,
        },
      );
    }
    ...
    if (error.code === 'ERR_CHILD_PROCESS_STDIO_MAXBUFFER' || (error.message && error.message.includes('maxBuffer'))) {
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `qsv flatten returned more data than could fit into memory. Specify an 'Output File Path' to stream results directly to disk instead.`,
        },
      );
    }
    ...
    if (
      ...
      throw new NodeOperationError(
        this.getNode(),
        `Operation 'flatten' is not available in the installed QSV binary`,
        {
          itemIndex,
          description: `The installed QSV binary at '${qsvBin}' does not include the 'flatten' feature. This feature requires a QSV build with the corresponding Cargo feature enabled (or 'all_features'). See https://github.com/dathere/qsv/blob/master/docs/help/flatten.md and https://github.com/dathere/qsv#feature-flags`,
        },
      );
    }
    ...
    if (rawError.includes('No such file or directory') || rawError.includes('os error 2')) {
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `qsv flatten could not find the file at '${inputPath}'. Check for typos, or if n8n is running in Docker, ensure the host directory is mounted into the container.`,
        },
      );
    }
    ...
    if (
      ...
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `qsv flatten was denied read access to '${inputPath}'. On macOS, check Full Disk Access or Removable Volumes permissions for the application running n8n.`,
        },
      );
    }
    ...
    throw new NodeOperationError(
      this.getNode(),
      `Failed executing 'qsv flatten': ${rawError}`,
      { itemIndex },
    );
  }
}
