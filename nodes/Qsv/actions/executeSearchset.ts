...
export async function executeSearchset(
  ...
  try {
    ...
    try {
      const val = this.getNodeParameter('invertMatch', itemIndex, false) as boolean;
      if (val) {
    ...
    try {
      const val = this.getNodeParameter('flagMatchesOnly', itemIndex, false) as boolean;
      if (val) {
    ...
    try {
      const val = this.getNodeParameter('unmatchedOutput', itemIndex, '') as string;
      if (val !== undefined && val !== '') {
