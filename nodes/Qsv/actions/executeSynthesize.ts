  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `Please ensure 'qsv' is installed and available in the system PATH where n8n is running, or specify its absolute path via the DARTFX_QSV_BIN_PATH environment variable. (Docs: https://github.com/dathere/qsv/blob/master/docs/help/synthesize.md)`,
        },
      );
    }
    ...
    if (
      ...
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `The installed QSV binary at '${qsvBin}' does not include the 'synthesize' feature. This command requires the 'synthesize' Cargo feature in QSV. This feature requires a QSV build with the corresponding Cargo feature enabled (or 'all_features'). See https://github.com/dathere/qsv/blob/master/docs/help/synthesize.md and https://github.com/dathere/qsv#feature-flags`,
        },
      );
    }
