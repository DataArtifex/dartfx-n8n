...
export async function executeTo(
  ...
  if (!format || !String(format).trim()) {
    throw new NodeOperationError(
      this.getNode(),
      'Parameter "Target Format" is required for to.',
      { itemIndex },
    );
  }
  if (!destination || !String(destination).trim()) {
    throw new NodeOperationError(
      this.getNode(),
      'Parameter "Destination" is required for to.',
      { itemIndex },
    );
  }
  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `Please ensure 'qsv' is installed and available in the system PATH where n8n is running, or specify its absolute path via the DARTFX_QSV_BIN_PATH environment variable. (Docs: https://github.com/dathere/qsv/blob/master/docs/help/to.md)`,
        },
      );
    }
    ...
    if (
      ...
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `The installed QSV binary at '${qsvBin}' does not include the 'to' feature. This command requires the 'to' Cargo feature in QSV. This feature requires a QSV build with the corresponding Cargo feature enabled (or 'all_features'). See https://github.com/dathere/qsv/blob/master/docs/help/to.md and https://github.com/dathere/qsv#feature-flags`,
        },
      );
    }
