...
export async function executePrompt(
  ...
  if (options.msg !== undefined && options.msg !== "") {
    args.push("--msg", String(options.msg));
  }
  if (options.filters !== undefined && options.filters !== "") {
    args.push("--filters", String(options.filters));
  }
  if (options.workdir !== undefined && options.workdir !== "") {
    args.push("--workdir", String(options.workdir));
  }
  if (options.fdOutput === true) {
    args.push("--fd-output");
  }
  if (options.saveFname !== undefined && options.saveFname !== "") {
    args.push("--save-fname", String(options.saveFname));
  }
  if (options.baseDelayMs !== undefined && options.baseDelayMs !== "") {
    args.push("--base-delay-ms", String(options.baseDelayMs));
  }
  ...
  try {
    ...
    if (
      ...
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `qsv prompt returned more data than could fit into memory. Specify an 'Output File Path' to stream results directly to disk instead.`,
        },
      );
    }
    ...
    if (
      ...
      throw new NodeOperationError(
        this.getNode(),
        `Operation 'prompt' is not available in the installed QSV binary`,
        {
          itemIndex,
          description: `The installed QSV binary at '${qsvBin}' does not include the 'prompt' feature. This feature may require a full feature build of QSV (e.g. qsv with all_features or a prebuilt binary with feature flags enabled). See https://github.com/dathere/qsv#feature-flags`,
        },
      );
    }
    ...
    if (
      ...
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `qsv prompt could not find the file at '${inputPath}'. Check for typos, or if n8n is running in Docker, ensure the host directory is mounted into the container.`,
        },
      );
    }
    ...
    if (
      ...
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `qsv prompt was denied read access to '${inputPath}'. On macOS, check Full Disk Access or Removable Volumes permissions for the application running n8n.`,
        },
      );
    }
    ...
    throw new NodeOperationError(
      this.getNode(),
      `Failed executing 'qsv prompt': ${rawError}`,
      { itemIndex },
    );
  }
}
