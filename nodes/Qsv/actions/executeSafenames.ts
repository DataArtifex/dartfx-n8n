...
/**
 * Action runner for 'qsv safenames'
...
export async function executeSafenames(
  ...
  try {
      const val = this.getNodeParameter('mode', itemIndex, '') as string;
    ...
    try {
      const val = this.getNodeParameter('reserved', itemIndex, '') as string;
      if (val !== undefined && val !== '') {
        args.push('--reserved', val);
      }
    } catch {}

    try {
      const val = this.getNodeParameter('prefix', itemIndex, '') as string;
      if (val !== undefined && val !== '') {
    ...
    try {
      const val = this.getNodeParameter('collapse', itemIndex, false) as boolean;
      if (val) {
        args.push('--collapse');
      }
    } catch {}
