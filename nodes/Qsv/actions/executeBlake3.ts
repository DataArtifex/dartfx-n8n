  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `Please ensure 'qsv' is installed and available in the system PATH where n8n is running, or specify its absolute path via the DARTFX_QSV_BIN_PATH environment variable. (Docs: https://github.com/dathere/qsv/blob/master/docs/help/blake3.md)`,
        },
      );
    }
    ...
    if (
      ...
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `The installed QSV binary at '${qsvBin}' does not include the 'blake3' feature. This feature requires a QSV build with the corresponding Cargo feature enabled (or 'all_features'). See https://github.com/dathere/qsv/blob/master/docs/help/blake3.md and https://github.com/dathere/qsv#feature-flags`,
        },
      );
    }
    ...
    if (rawError.includes('No such file or directory') || rawError.includes('os error 2')) {
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `qsv blake3 could not find the file at '${inputPath}'. Check for typos, or if n8n is running in Docker, ensure the host directory is mounted into the container.`,
        },
      );
    }
    ...
    if (
      ...
      throw new NodeOperationError(
        ...
        {
          itemIndex,
          description: `qsv blake3 was denied read access to '${inputPath}'. On macOS, check Full Disk Access or Removable Volumes permissions for the application running n8n.`,
        },
      );
    }
    ...
    throw new NodeOperationError(
      this.getNode(),
      `Failed executing 'qsv blake3': ${rawError}`,
      { itemIndex },
    );
  }
}
