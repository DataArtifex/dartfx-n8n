import type { INodeProperties } from "n8n-workflow";

/**
 * Property definitions for 'qsv scoresql' operation.
 * Auto-generated by scripts/generate-qsv-nodes.ts
 */
export const ScoresqlDescription: INodeProperties[] = [
  {
    displayName: "Input CSV File Path",
    name: "inputPath",
    type: "string",
    default: "",
    required: true,
    displayOptions: {
      show: {
        operation: ["scoresql"],
      },
    },
    description: "The absolute or relative file path to the input CSV file",
  },
  {
    displayName: "Output File Path",
    name: "outputPath",
    type: "string",
    default: "",
    displayOptions: {
      show: {
        operation: ["scoresql"],
      },
    },
    description:
      "Path where the output will be saved. If empty, result may be returned in stdout/JSON.",
  },
  {
    displayName: "Json",
    name: "json",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["scoresql"],
      },
    },
    description: "Output results as JSON instead of human-readable report.",
  },
  {
    displayName: "Duckdb",
    name: "duckdb",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["scoresql"],
      },
    },
    description:
      'Use DuckDB for query plan analysis instead of Polars. Uses the QSV_DUCKDB_PATH environment variable if set, otherwise looks for "duckdb" in PATH.',
  },
  {
    displayName: "Try Parsedates",
    name: "tryParsedates",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["scoresql"],
      },
    },
    description: "Automatically try to parse dates/datetimes and time.",
  },
  {
    displayName: "Infer Len",
    name: "inferLen",
    type: "string",
    default: "10000",
    displayOptions: {
      show: {
        operation: ["scoresql"],
      },
    },
    description:
      "Number of rows to scan when inferring schema. [default: 10000]",
  },
  {
    displayName: "Ignore Errors",
    name: "ignoreErrors",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["scoresql"],
      },
    },
    description: "Ignore errors when parsing CSVs.",
  },
  {
    displayName: "Truncate Ragged Lines",
    name: "truncateRaggedLines",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["scoresql"],
      },
    },
    description: "Truncate lines with more fields than the header.",
  },
  {
    displayName: "Delimiter",
    name: "delimiter",
    type: "string",
    default: ",",
    displayOptions: {
      show: {
        operation: ["scoresql"],
      },
    },
    description:
      "The field delimiter for reading CSV data. Must be a single character. [default: ,]",
  },
  {
    displayName: "Quiet",
    name: "quiet",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["scoresql"],
      },
    },
    description: "Do not print informational messages to stderr.",
  },
];
