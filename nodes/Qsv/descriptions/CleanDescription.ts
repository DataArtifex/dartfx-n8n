import type { INodeProperties } from 'n8n-workflow';

/**
 * Property definitions for 'qsv clean' operation.
 * Auto-generated by scripts/generate-qsv-nodes.ts
 */
export const CleanDescription: INodeProperties[] = [
    {
      displayName: 'Input CSV File Path',
      name: 'inputPath',
      type: 'string',
      default: '',
      required: true,
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'The absolute or relative file path to the input CSV file',
    },
    {
      displayName: 'Output File Path',
      name: 'outputPath',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'Path where the output will be saved. If empty, result may be returned in stdout/JSON.',
    },
    {
      displayName: 'Stale',
      name: 'stale',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'Only remove STALE or ORPHANED caches: a cache whose source file is newer than the cache, or whose source file no longer exists.',
    },
    {
      displayName: 'Recursive',
      name: 'recursive',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'Recurse into subdirectories when <input> is a directory.',
    },
    {
      displayName: 'Dry Run',
      name: 'dryRun',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'Preview only; never delete. Wins over --force if both are given (this is also the default when neither is set).',
    },
    {
      displayName: 'Force',
      name: 'force',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'Actually delete the files.',
    },
    {
      displayName: 'Index',
      name: 'index',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'Index (.idx) caches. [default category]',
    },
    {
      displayName: 'Stats',
      name: 'stats',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'Stats caches. [default category]',
    },
    {
      displayName: 'Frequency',
      name: 'frequency',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'Frequency caches. [default category]',
    },
    {
      displayName: 'Schema',
      name: 'schema',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'schema .schema.json / .pschema.json outputs.',
    },
    {
      displayName: 'Validate',
      name: 'validate',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'validate .valid / .invalid / .validation-errors.tsv outputs.',
    },
    {
      displayName: 'Moarstats',
      name: 'moarstats',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'moarstats .stats.bivariate[.joined].csv outputs.',
    },
    {
      displayName: 'All',
      name: 'all',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'Select every category above.',
    },
    {
      displayName: 'Quiet',
      name: 'quiet',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['clean'],
        },
      },
      description: 'Do not print the per-file / summary report.',
    },
];
