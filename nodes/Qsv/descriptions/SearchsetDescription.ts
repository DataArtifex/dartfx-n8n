import type { INodeProperties } from "n8n-workflow";

/**
 * Property definitions for 'qsv searchset' operation.
 * Auto-generated by scripts/generate-qsv-nodes.ts
 */
export const SearchsetDescription: INodeProperties[] = [
  {
    displayName: "Input CSV File Path",
    name: "inputPath",
    type: "string",
    default: "",
    required: true,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description: "The absolute or relative file path to the input CSV file",
  },
  {
    displayName: "Output File Path",
    name: "outputPath",
    type: "string",
    default: "",
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "Path where the output will be saved. If empty, result may be returned in stdout/JSON.",
  },
  {
    displayName: "Ignore Case",
    name: "ignoreCase",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "Case insensitive search. This is equivalent to prefixing the regex with '(?i)'.",
  },
  {
    displayName: "Literal",
    name: "literal",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "Treat the regex as a literal string. This allows you to search for matches that contain regex special characters.",
  },
  {
    displayName: "Exact",
    name: "exact",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "Match the ENTIRE field exactly. Treats the pattern as a literal string (like --literal) and automatically anchors it to match the complete field value (^pattern$).",
  },
  {
    displayName: "Select",
    name: "select",
    type: "string",
    default: "",
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "Select the columns to search. See 'qsv select -h' for the full syntax.",
  },
  {
    displayName: "Invert Match",
    name: "invertMatch",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description: "Select only rows that did not match",
  },
  {
    displayName: "Unicode",
    name: "unicode",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "Enable unicode support. When enabled, character classes will match all unicode word characters instead of only ASCII word characters. Decreases performance.",
  },
  {
    displayName: "Flag",
    name: "flag",
    type: "string",
    default: "",
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "If given, the command will not filter rows but will instead flag the found rows in a new column named <column>. For each found row, <column> is set to the row number of the row, followed by a semicolon, then a list of the matching regexes.",
  },
  {
    displayName: "Flag Matches Only",
    name: "flagMatchesOnly",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "When --flag is enabled, only rows that match are sent to output. Rows that do not match are filtered.",
  },
  {
    displayName: "Unmatched Output",
    name: "unmatchedOutput",
    type: "string",
    default: "",
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "When --flag-matches-only is enabled, output the rows that did not match to <file>.",
  },
  {
    displayName: "Quick",
    name: "quick",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "Return on first match with an exitcode of 0, returning the row number of the first match to stderr. Return exit code 1 if no match is found. No output is produced. Ignored if --json is enabled.",
  },
  {
    displayName: "Count",
    name: "count",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "Write the number of matches to stderr. Suppressed by --quiet. Ignored if --json is enabled.",
  },
  {
    displayName: "Json",
    name: "json",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "Return number of matches, number of rows with matches, and number of rows to stderr in JSON format.",
  },
  {
    displayName: "Size Limit",
    name: "sizeLimit",
    type: "string",
    default: "50",
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "Set the approximate size limit (MB) of the compiled regular expression. If the compiled expression exceeds this number, then a compilation error is returned. Modify this only if you're getting regular expression compilation errors. [default: 50]",
  },
  {
    displayName: "Dfa Size Limit",
    name: "dfaSizeLimit",
    type: "string",
    default: "10",
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "Set the approximate size of the cache (MB) used by the regular expression engine's Discrete Finite Automata. Modify this only if you're getting regular expression compilation errors. [default: 10]",
  },
  {
    displayName: "Not One",
    name: "notOne",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description: "Use exit code 0 instead of 1 for no match found.",
  },
  {
    displayName: "Jobs",
    name: "jobs",
    type: "string",
    default: "",
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "The number of jobs to run in parallel when the given CSV data has an index. Note that a file handle is opened for each job. When not set, defaults to the number of CPUs detected.",
  },
  {
    displayName: "No Headers",
    name: "noHeaders",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "When set, the first row will not be interpreted as headers. (i.e., They are not searched, analyzed, sliced, etc.)",
  },
  {
    displayName: "Delimiter",
    name: "delimiter",
    type: "string",
    default: "",
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "The field delimiter for reading CSV data. Must be a single character. (default: ,)",
  },
  {
    displayName: "Progressbar",
    name: "progressbar",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description: "Show progress bars. Not valid for stdin.",
  },
  {
    displayName: "Quiet",
    name: "quiet",
    type: "boolean",
    default: false,
    displayOptions: {
      show: {
        operation: ["searchset"],
      },
    },
    description:
      "Do not write the match count (--count) or the first match row number reported by --quick to stderr. Does not suppress the --json summary.",
  },
];
