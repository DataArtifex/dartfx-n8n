import type { INodeProperties } from 'n8n-workflow';

/**
 * Property definitions for 'qsv to' operation.
 * Auto-generated by scripts/generate-qsv-nodes.ts
 */
export const ToDescription: INodeProperties[] = [
    {
      displayName: 'Input CSV File Path',
      name: 'inputPath',
      type: 'string',
      default: '',
      required: true,
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'The absolute or relative file path to the input CSV file',
    },
    {
      displayName: 'Output File Path',
      name: 'outputPath',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'Path where the output will be saved. If empty, result may be returned as JSON/stdout.',
    },
    {
      displayName: 'Print Package',
      name: 'printPackage',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'Print statistics as datapackage, by default will print field summary.',
    },
    {
      displayName: 'Dump',
      name: 'dump',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'Create database dump file for use with `psql` or `sqlite3` command line tools',
    },
    {
      displayName: 'Stats',
      name: 'stats',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'Produce extra statistics about the data beyond just type guessing.',
    },
    {
      displayName: 'Stats Csv',
      name: 'statsCsv',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'Output stats as CSV to specified file.',
    },
    {
      displayName: 'Quiet',
      name: 'quiet',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'Do not print out field summary.',
    },
    {
      displayName: 'Schema',
      name: 'schema',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'The schema to load the data into. (postgres only).',
    },
    {
      displayName: 'Infer Len',
      name: 'inferLen',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'The number of rows to use for schema inference (parquet only).',
    },
    {
      displayName: 'Try Parse Dates',
      name: 'tryParseDates',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'Attempt to parse date/datetime columns with polars\' date inference logic.',
    },
    {
      displayName: 'Drop',
      name: 'drop',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'Drop tables before loading new data into them (postgres/sqlite only).',
    },
    {
      displayName: 'Evolve',
      name: 'evolve',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'If loading into existing db, alter existing tables so that new data will load.',
    },
    {
      displayName: 'Pipe',
      name: 'pipe',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'Adjust output format for piped data (omits row counts and field format columns).',
    },
    {
      displayName: 'Table',
      name: 'table',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'Use this as the table/sheet/file name (postgres/sqlite/xlsx/ods/parquet).',
    },
    {
      displayName: 'Separator',
      name: 'separator',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'For xlsx, use this character to help truncate xlsx sheet names.',
    },
    {
      displayName: 'Compression',
      name: 'compression',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'Parquet compression codec (parquet only).',
    },
    {
      displayName: 'Compress Level',
      name: 'compressLevel',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'Compression level (parquet only). For gzip: 1-9 (default: 6). For zstd: -7 to 22 (default: 3). Ignored for other codecs.',
    },
    {
      displayName: 'All Strings',
      name: 'allStrings',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'Convert all fields to strings.',
    },
    {
      displayName: 'Jobs',
      name: 'jobs',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'The number of jobs to run in parallel.',
    },
    {
      displayName: 'Delimiter',
      name: 'delimiter',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['to'],
        },
      },
      description: 'The field delimiter for reading CSV data.',
    },
];
