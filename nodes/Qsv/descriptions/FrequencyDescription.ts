import type { INodeProperties } from 'n8n-workflow';

/**
 * Property definitions for 'qsv frequency' operation.
 * Auto-generated by scripts/generate-qsv-nodes.ts
 */
export const FrequencyDescription: INodeProperties[] = [
    {
      displayName: 'Input CSV File Path',
      name: 'inputPath',
      type: 'string',
      default: '',
      required: true,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'The absolute or relative file path to the input CSV file',
    },
    {
      displayName: 'Output File Path',
      name: 'outputPath',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Path where the output will be saved. If empty, result may be returned as JSON/stdout.',
    },
    {
      displayName: 'Select',
      name: 'select',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Select a subset of columns to compute frequencies',
    },
    {
      displayName: 'Limit',
      name: 'limit',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Limit the frequency table to the N most common',
    },
    {
      displayName: 'Sketch Method',
      name: 'sketchMethod',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Algorithm used to compute the frequency table.',
    },
    {
      displayName: 'Sketch Map Size',
      name: 'sketchMapSize',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Maximum map size for the Frequent Items sketch.',
    },
    {
      displayName: 'Unq Limit',
      name: 'unqLimit',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'If a column has all unique values, limit the',
    },
    {
      displayName: 'Lmt Threshold',
      name: 'lmtThreshold',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'The threshold for which --limit and --unq-limit',
    },
    {
      displayName: 'Rank Strategy',
      name: 'rankStrategy',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'The strategy to use when there are count-tied values in the frequency table.',
    },
    {
      displayName: 'Pct Dec Places',
      name: 'pctDecPlaces',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'The number of decimal places to round the percentage to.',
    },
    {
      displayName: 'Other Sorted',
      name: 'otherSorted',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'By default, the "Other" category is placed at the',
    },
    {
      displayName: 'Other Text',
      name: 'otherText',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'The text to use for the "Other" category. If set to the',
    },
    {
      displayName: 'No Other',
      name: 'noOther',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Don\'t include the "Other" category in the frequency table.',
    },
    {
      displayName: 'Null Sorted',
      name: 'nullSorted',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'By default, the NULL category (controlled by --null-text)',
    },
    {
      displayName: 'Asc',
      name: 'asc',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Sort the frequency tables in ascending order by count.',
    },
    {
      displayName: 'No Trim',
      name: 'noTrim',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Don\'t trim whitespace from values when computing frequencies.',
    },
    {
      displayName: 'Null Text',
      name: 'nullText',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'The text to use for NULL values. If set to the literal',
    },
    {
      displayName: 'No Nulls',
      name: 'noNulls',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Don\'t include NULLs in the frequency table.',
    },
    {
      displayName: 'Pct Nulls',
      name: 'pctNulls',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Include NULL values in percentage and rank calculations.',
    },
    {
      displayName: 'Ignore Case',
      name: 'ignoreCase',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Ignore case when computing frequencies.',
    },
    {
      displayName: 'No Float',
      name: 'noFloat',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Exclude Float columns from frequency analysis.',
    },
    {
      displayName: 'Stats Filter',
      name: 'statsFilter',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Filter columns based on their statistics using a Luau expression.',
    },
    {
      displayName: 'All Unique Text',
      name: 'allUniqueText',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'The text to use for the "<ALL_UNIQUE>" category.',
    },
    {
      displayName: 'Vis Whitespace',
      name: 'visWhitespace',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Visualize whitespace characters in the output. See',
    },
    {
      displayName: 'Jobs',
      name: 'jobs',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'The number of jobs to run in parallel when the given CSV data has',
    },
    {
      displayName: 'Frequency Jsonl',
      name: 'frequencyJsonl',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Write the complete frequency distribution as a',
    },
    {
      displayName: 'High Card Threshold',
      name: 'highCardThreshold',
      type: 'string',
      default: '100',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Absolute cardinality threshold for HIGH_CARDINALITY classification in the frequency cache. Can also be set with QSV_FREQ_HIGH_CARD_THRESHOLD env var (env var takes precedence when CLI value equals the default). Only used with --frequency-jsonl. [default: 100]',
    },
    {
      displayName: 'High Card Pct',
      name: 'highCardPct',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Percentage of rowcount threshold for HIGH_CARDINALITY',
    },
    {
      displayName: 'Force',
      name: 'force',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Force recomputation even when a valid frequency cache',
    },
    {
      displayName: 'Json',
      name: 'json',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Output frequency table as nested JSON instead of CSV.',
    },
    {
      displayName: 'Pretty Json',
      name: 'prettyJson',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Same as --json but pretty prints the JSON output.',
    },
    {
      displayName: 'Toon',
      name: 'toon',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Output frequency table and select stats in TOON format instead of CSV.',
    },
    {
      displayName: 'No Stats',
      name: 'noStats',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'When using the JSON or TOON output mode, do not include the additional stats.',
    },
    {
      displayName: 'Weight',
      name: 'weight',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Compute weighted frequencies using the specified column as weights.',
    },
    {
      displayName: 'Output',
      name: 'output',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Write output to <file> instead of stdout.',
    },
    {
      displayName: 'No Headers',
      name: 'noHeaders',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'When set, the first row will NOT be included',
    },
    {
      displayName: 'Delimiter',
      name: 'delimiter',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'The field delimiter for reading CSV data.',
    },
    {
      displayName: 'Memcheck',
      name: 'memcheck',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['frequency'],
        },
      },
      description: 'Use CONSERVATIVE heuristics for the in-memory load',
    },
];
