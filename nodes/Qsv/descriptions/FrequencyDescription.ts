...
export const FrequencyDescription: INodeProperties[] = [
  ...
  },
  {
    ...
    type: 'string',
    default: '',
    description: 'Additional raw command line arguments to pass to qsv frequency (Docs: https://github.com/dathere/qsv/blob/master/docs/help/frequency.md)',
    displayOptions: {
      show: {
        operation: ['frequency'],
      },
  ...
  },
  {
    ...
    },
    {
      ...
      type: 'boolean',
      default: false,
      description: 'Force recomputation even when a valid frequency cache exists, bypassing the auto-reuse path. Also regenerates the cache when combined with --frequency-jsonl.',
    },
    {
    ...
    },
    {
      ...
      type: 'string',
      default: '',
      description: 'Percentage of rowcount threshold for HIGH_CARDINALITY classification in the frequency cache. Must be between 1 and 100. Can also be set with QSV_FREQ_HIGH_CARD_PCT env var (env var takes precedence when CLI value equals the default). Only used with --frequency-jsonl. [default: 90]',
    },
    {
      ...
      type: 'string',
      default: '',
      description: 'Absolute cardinality threshold for HIGH_CARDINALITY classification in the frequency cache. Can also be set with QSV_FREQ_HIGH_CARD_THRESHOLD env var (env var takes precedence when CLI value equals the default). Only used with --frequency-jsonl. [default: 100]',
    },
    {
    ...
    },
    {
      ...
      type: 'string',
      default: '',
      description: 'Exclude Float columns from frequency analysis. Floats typically contain continuous values where frequency tables are not meaningful. To exclude ALL Float columns, use --no-float "*" To exclude Floats except specific columns, specify a comma-separated list of Float columns to INCLUDE. e.g. "--no-float *" excludes all Floats "--no-float price,rate" excludes Floats except \'price\' and \'rate\' Requires stats cache for type detection.',
    },
    {
    ...
    },
    {
      ...
      type: 'boolean',
      default: false,
      description: 'Don\'t include NULLs in the frequency table. This is equivalent to --null-text "<NONE>".',
    },
    {
      ...
      type: 'boolean',
      default: false,
      description: 'Don\'t include the "Other" category in the frequency table. This is equivalent to --other-text "<NONE>".',
    },
    {
    ...
    },
    {
      ...
      type: 'boolean',
      default: false,
      description: 'By default, the NULL category (controlled by --null-text) is placed at the end of the frequency table for a field, after "Other" if present. If this is enabled, the NULL category will be sorted with the rest of the values by count.',
    },
    {
      ...
      type: 'string',
      default: '',
      description: 'The text to use for NULL values. If set to the literal string "<NONE>" (case-sensitive, exact match), NULLs will not be included in the frequency table (equivalent to --no-nulls). [default: (NULL)]',
    },
    {
    ...
    },
    {
      ...
      type: 'string',
      default: '',
      description: 'The text to use for the "Other" category. If set to the literal string "<NONE>" (case-sensitive, exact match), the "Other" category will not be included in the frequency table. [default: Other]',
    },
    {
    ...
    },
    {
      ...
      type: 'string',
      default: '',
      description: 'The strategy to use when there are count-tied values in the frequency table. See https://en.wikipedia.org/wiki/Ranking for more info. Valid values are: * dense: Assigns consecutive integers regardless of ties, incrementing by 1 for each new count value (AKA "1223" ranking). * min: Tied items receive the minimum rank position (AKA "1224" ranking). * max: Tied items receive the maximum rank position (AKA "1334" ranking). * ordinal: The next rank is the current rank plus 1 (AKA "1234" ranking). * average: Tied items receive the average of their ordinal positions (AKA "1 2.5 2.5 4" ranking). Note that tied values with the same rank are sorted alphabetically. [default: dense]',
    },
    {
      displayName: 'Select',
      name: 'select',
    ...
    },
    {
      ...
      type: 'string',
      default: '',
      description: 'Maximum map size for the Frequent Items sketch. Must be a power of two and at least 8. Larger values tighten error bounds at the cost of more memory. Only used when sketch-method is frequent_items. [default: 4096]',
    },
    {
    ...
    },
    {
      ...
      type: 'string',
      default: '',
      description: 'If a column has all unique values, limit the frequency table to a sample of N unique items. Set to \'0\' to disable a unique_limit. Only applies in unweighted mode; ignored when --weight is set. [default: 10]',
    },
    {
