import type { INodeProperties } from 'n8n-workflow';

/**
 * Property definitions for 'qsv sample' operation.
 * Auto-generated by scripts/generate-qsv-nodes.ts
 */
export const SampleDescription: INodeProperties[] = [
    {
      displayName: 'Input CSV File Path',
      name: 'inputPath',
      type: 'string',
      default: '',
      required: true,
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'The absolute or relative file path to the input CSV file',
    },
    {
      displayName: 'Output File Path',
      name: 'outputPath',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Path where the output will be saved. If empty, result may be returned as JSON/stdout.',
    },
    {
      displayName: 'Seed',
      name: 'seed',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Random Number Generator (RNG) seed.',
    },
    {
      displayName: 'Rng',
      name: 'rng',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'The Random Number Generator (RNG) algorithm to use.',
    },
    {
      displayName: 'Bernoulli',
      name: 'bernoulli',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Use Bernoulli sampling instead of indexed or reservoir sampling.',
    },
    {
      displayName: 'Systematic',
      name: 'systematic',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Use systematic sampling (every nth record as specified by <sample-size>).',
    },
    {
      displayName: 'Stratified',
      name: 'stratified',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Use stratified sampling. The strata column is specified by <col>.',
    },
    {
      displayName: 'Weighted',
      name: 'weighted',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Use weighted sampling. The weight column is specified by <col>.',
    },
    {
      displayName: 'Varopt',
      name: 'varopt',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Use VAROPT weighted reservoir sampling (A-ExpJ keying).',
    },
    {
      displayName: 'Mergeable Reservoir',
      name: 'mergeableReservoir',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Use a mergeable Algorithm-R reservoir sampler. Distribution is',
    },
    {
      displayName: 'Cluster',
      name: 'cluster',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Use cluster sampling. The cluster column is specified by <col>.',
    },
    {
      displayName: 'Timeseries',
      name: 'timeseries',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Use time-series sampling. The time column is specified by <col>.',
    },
    {
      displayName: 'Ts Interval',
      name: 'tsInterval',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Time interval for grouping records. Format: <number><unit>',
    },
    {
      displayName: 'Ts Start',
      name: 'tsStart',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Starting point for time-series sampling.',
    },
    {
      displayName: 'Ts Adaptive',
      name: 'tsAdaptive',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Adaptive sampling mode for time-series data.',
    },
    {
      displayName: 'Ts Aggregate',
      name: 'tsAggregate',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Aggregation function to apply within each time interval.',
    },
    {
      displayName: 'Ts Input Tz',
      name: 'tsInputTz',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Timezone for parsing input timestamps. Can be an IANA timezone name or "local" for the local timezone.',
    },
    {
      displayName: 'Ts Prefer Dmy',
      name: 'tsPreferDmy',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Prefer to parse dates in dmy format. Otherwise, use mdy format.',
    },
    {
      displayName: 'Sketch Out',
      name: 'sketchOut',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'After sampling, also write a binary sketch describing the internal',
    },
    {
      displayName: 'Sketch In',
      name: 'sketchIn',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Comma-separated list of sketch files produced by --sketch-out.',
    },
    {
      displayName: 'User Agent',
      name: 'userAgent',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Specify custom user agent to use when the input is a URL.',
    },
    {
      displayName: 'Timeout',
      name: 'timeout',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Inactivity timeout for downloading URLs in seconds. Aborts only if',
    },
    {
      displayName: 'Max Size',
      name: 'maxSize',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Maximum size of the file to download in MB before sampling.',
    },
    {
      displayName: 'Force',
      name: 'force',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Do not use stats cache, even if its available.',
    },
    {
      displayName: 'Output',
      name: 'output',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'Write output to <file> instead of stdout.',
    },
    {
      displayName: 'No Headers',
      name: 'noHeaders',
      type: 'boolean',
      default: false,
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'When set, the first row will be considered as part of',
    },
    {
      displayName: 'Delimiter',
      name: 'delimiter',
      type: 'string',
      default: '',
      displayOptions: {
        show: {
          operation: ['sample'],
        },
      },
      description: 'The field delimiter for reading/writing CSV data.',
    },
];
