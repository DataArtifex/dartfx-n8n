...
export const VizDescription: INodeProperties[] = [
  ...
  },
  {
    ...
    description:
      "The z column: a heatmap pivot value (with --x and --y), or the third numeric axis for scatter3d.",
  },
  {
    displayName: "Cols",
    name: "cols",
  ...
  },
  {
    ...
    description:
      "Column to split into multiple series (one trace per distinct value). Applies to bar, line, scatter, scatter3d, radar, map and geo.",
  },
  {
    displayName: "Color",
    name: "color",
    ...
    description:
      "For scatter/scatter3d/map/geo: a numeric column to encode as marker color (a continuous colorscale with a colorbar). For categorical coloring, use the --series option instead. Cannot be combined with --series. In map density mode, this column is the heatmap weight.",
  },
  {
    displayName: "Size",
    name: "size",
    ...
    description:
      "For scatter/scatter3d/map/geo: a numeric column to encode as marker size, producing a bubble chart (values are rescaled to a readable pixel range). Cannot be combined with --series, EXCEPT on an animated scatter (see the --slider option), where the --series column names the entity and this column is the third data variable. In map density mode, this column is the heatmap weight.",
  },
  {
    displayName: "Donut",
    name: "donut",
  ...
  },
  {
    ...
    description:
      "Number of bins. For histogram: bins along the x-axis (default: auto). For contour: the per-axis resolution of the density grid (default: 20).",
  },
  {
    displayName: "Agg",
    name: "agg",
    ...
    description:
      "For bar/line, aggregate the y values when the x value repeats. One of: sum, mean, count, min, max. For treemap/sunburst/icicle, only additive aggregations apply: count (default) or sum (requires --value).",
  },
  {
    ...
    description:
      "Which sample points to draw alongside a box. Reading the raw values lets plotly render true Tukey whiskers (1.5*IQR)",
  },
  {
    displayName: "Lat",
    name: "lat",
  ...
  },
  {
    ...
    description:
      "Render a density heatmap (DensityMap) instead of points. Weighted by the --color or --size column when given, else by a uniform weight. Cannot be combined with --series.",
  },
  {
    displayName: "Style",
    name: "style",
  ...
  },
  {
    ...
    description:
      "Map projection for `viz geo`. One of: natural-earth (the default), mercator, orthographic, equirectangular, albers-usa, robinson, winkel-tripel, mollweide, hammer, azimuthal-equal-area. `viz geo` also reuses the lat, lon, text, color, size and series options from `map`. [default: natural-earth]",
  },
  {
    ...
    description:
      "Column holding the region key for each row (an ISO-3 country code, a 2-letter US state code, a country name, or a GeoJSON feature id, per --location-mode). With --geocode, this instead names a place-name column to forward-geocode into region codes.",
  },
  {
    ...
    description:
      "How --locations values are matched to regions. One of: iso3 (the default, ISO-3166-1 alpha-3 country codes), usa-states (2-letter US state codes), country-names (full country names), geojson-id (match a --geojson feature id). [default: iso3]",
  },
  {
    ...
    description:
      "Colorscale for the region fill. One of: viridis (the default), cividis, greys, greens, blues, reds, ylgnbu, ylorrd, bluered, rdbu, portland, electric, jet, hot, blackbody, earth, picnic, rainbow. [default: viridis]",
  },
  {
    displayName: "Map",
    name: "map",
    ...
    description:
      "Render on a token-free MapLibre tile basemap (a ChoroplethMap) instead of the default projection basemap. Requires --geojson; feature-id-key defaults to id. Reuses --style for the basemap.",
  },
  {
    displayName: "Geojson",
    name: "geojson",
  ...
  },
  {
    ...
    description:
      "Property path in each GeoJSON feature whose value matches an entry in the locations column, or that labels each binned region (e.g. id, properties.fips). [default: id]",
  },
  {
    ...
    description:
      "GeoJSON property path whose value is shown as the human-readable region label in choropleth hover (e.g. properties.name). When omitted, common name keys are auto-detected; falls back to the feature id when absent.",
  },
  {
  ...
  },
  {
    ...
    description:
      "Derive the region codes by reusing qsv's geocode engine (needs a build with the geocode feature). Either reverse-geocode the lat/lon points, or forward-geocode the locations name column. Only valid with location modes iso3 or usa-states. `viz choropleth` also reuses --value, --agg, --style and the lat/lon options.",
  },
  {
    ...
    description:
      "Column holding each row's expected country, as an ISO-3166-1 alpha-2 code (e.g. US, GB, BR). Without it a place-name search spans every country, so US city names have resolved to Pakistan and the UK, and a point near a border takes the nearest FOREIGN place. Rows whose match falls outside the hint are counted and reported, never silently dropped. Country NAMES are not accepted, only codes. With location-mode usa-states the country defaults to `us`, so no hint is needed there. Applies to both geocode sources.",
  },
  {
    ...
    description:
      "Column holding each row's expected admin1 (state/province), either as a qualified Geonames code (US.AL) or a bare subdivision code (AL) qualified by the country. Narrows a place-name search within a country, for names that are ambiguous on their own (there are dozens of US Springfields). Spelled-out names are rejected: admin1 names are matched by prefix, so `AL` would silently accept Alaska. Valid only with a locations name column, not with lat/lon points.",
  },
  {
    displayName: "No Snap",
    name: "noSnap",
  ...
  },
  {
    ...
    description:
      "Number of columns in the Data Schematic grid for the per-column distribution panels. Overview panels (map/geo, correlation, time-series) always span the full width. [default: 2]",
  },
  {
  ...
  },
  {
    ...
    description:
      'For the `viz smart` map panel: whether to offer an in-map "Clusters/Points" toggle that collapses dense points into native MapLibre count bubbles (which expand back into individual, hoverable points on zoom-in). Hovering a bubble shows its point count; clicking it zooms to where that cluster breaks apart. The map always OPENS as individual points; the toggle switches clustering on. One of: auto, on, off. "auto" (the default) offers the toggle for a plain-marker core (no density heatmap, no bubble-size measure) once it reaches 1,000 points. "on" offers it whenever the core draws as markers, regardless of point count or a bubble-size measure. "off" omits the toggle, so points are always drawn plainly. Only affects `smart`. [default: auto]',
  },
  {
    displayName: "Photos",
    name: "photos",
  ...
  },
  {
    ...
    description:
      'Omit the "(NULL)" bar (empty cells) from frequency bar charts. By default `viz smart` shows a "(NULL)" bar, like `qsv frequency`.',
  },
  {
    ...
    description:
      'Omit the "Other (N)" aggregate bar from frequency bar charts. It collects the categories beyond --limit (N = how many distinct categories were rolled up) and is shown by default. When just ONE category is left over, it is charted under its own name instead of as an opaque "Other (1)" bar.',
  },
  {
    displayName: "Smarter",
    name: "smarter",
  ...
  },
  {
    ...
    description:
      "For `smart`, the chart used for the categorical part-to-whole hierarchy panel (built when 2+ low-cardinality dimensions exist). One of: auto (default), treemap, sunburst, icicle. auto follows best practice — a treemap for a shallow 2-level hierarchy (accurate size comparison) and a sunburst for a deep 3-level one (parent child structure); icicle is an opt-in level-aligned alternative. Only affects `smart`.",
  },
  {
  ...
  },
  {
    ...
    description:
      "Add a draggable range-slider (a navigator strip) under the x-axis for panning and zooming. Best for an ordered x-axis (time series, line, candlestick/ohlc). Applies to cartesian charts (bar/line/scatter/histogram/box/violin/candlestick/ ohlc); off by default.",
  },
  {
    displayName: "Slider",
    name: "slider",
  ...
  },
  {
    ...
    description:
      "Milliseconds each animation frame is shown while playing. [default: 800]",
  },
  {
  ...
  },
  {
    ...
    description:
      "Caption note drawn at the bottom of the plot (e.g. to note a clipped outlier). Cartesian charts only.",
  },
  {
    displayName: "Theme",
    name: "theme",
    ...
    description:
      "Plotly theme that drives the chart's overall look (background, fonts, axis styling). One of: default, plotly_white, plotly_dark, seaborn, seaborn_whitegrid, seaborn_dark, matplotlib, plotnine (case-insensitive; hyphens accepted). When omitted, qsv's built-in look is used. Applies to all chart types, including `smart`.",
  },
  {
    ...
    description:
      "Render the Data Schematic UI in this language. Accepts a BCP-47/ISO 639 code (es, spa) or an English language name (Spanish). Overrides the language detected in a data dictionary, and is also forwarded to describegpt when inferring one. Curated: en, es, fr, de, it, pt-BR, ja, zh-CN. [default: auto]",
  },
  {
    displayName: "Width",
    name: "width",
    ...
    description:
      "Image width in pixels for static export. Default 1000; for `smart`, auto-scaled to the grid's column count.",
  },
  {
    displayName: "Height",
    name: "height",
    ...
    description:
      "Image height in pixels for static export. Default 600; for `smart`, auto-scaled to the number of panel rows.",
  },
  {
    displayName: "Scale",
    name: "scale",
  ...
  },
  {
    ...
    description:
      "When set, the first row will not be interpreted as headers. Columns can then only be selected by index.",
  },
];
